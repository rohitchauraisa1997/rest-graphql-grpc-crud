...
type VideoRepository interface {
	...
	UpdateVideo(id string, updatedVideo *model.Video) *model.Video
}
...
func (db *database) UpdateVideo(id string, newInputVideo *model.Video) *model.Video {
	collection := db.Client.Database(DATABASE).Collection(COLL)
	filter := bson.M{"_id": id}

	// Define the update operation
	update := bson.M{"$set": bson.M{"title": newInputVideo.Title, "url": newInputVideo.URL}}
